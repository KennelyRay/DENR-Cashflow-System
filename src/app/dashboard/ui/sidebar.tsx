  ...
  return (
    <>
      ...
      <button 
        ...
        className="fixed top-4 left-4 z-50 lg:hidden p-2 rounded-md bg-white text-slate-600 shadow-sm border border-slate-200 hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-emerald-500"
      >
      ...
      {/* Sidebar Container */}
      <div className={`fixed inset-y-0 left-0 z-40 flex w-64 flex-col border-r border-slate-200 bg-white transition-transform duration-300 ease-in-out lg:static lg:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        {/* Sidebar Header / Logo */}
        <div className="flex h-16 shrink-0 items-center gap-3 border-b border-slate-100 px-6 mt-14 lg:mt-0">
          ...
          <span className="font-bold tracking-tight text-teal-800">
            DENR Cashflow
          </span>
        </div>
      ...
      <nav className="flex flex-1 flex-col gap-1 overflow-y-auto p-4">
        {activeProfile ? (
          <>
            <div className="mb-4 rounded-lg bg-slate-50 p-3 ring-1 ring-inset ring-slate-200">
              <p className="text-xs font-semibold text-slate-500">Current PaP Profile</p>
              <p className="mt-1 truncate text-sm font-bold text-slate-900" title={activeProfile.name}>
                {activeProfile.name}
              </p>
              <Link 
